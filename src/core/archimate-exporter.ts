...
export function extractModel(files: VaultFile[], vaultName?: string): ArchiModel {
  ...
  return {
    ...
    documentation: `Exported from TOGAF vault "${vaultName || 'unknown'}" by AchiPilot`,
    elements,
    relationships,
    views,
    metadata: {
...
export function serializeToXml(model: ArchiModel): string {
  ...
  lines.push('    <item>');
  lines.push(`      <label xml:lang="en">AchiPilot Export</label>`);
  lines.push(`      <documentation xml:lang="en">Generated by AchiPilot v${model.metadata.generatorVersion} at ${model.metadata.exportedAt}</documentation>`);
  lines.push('    </item>');
  lines.push('  </organizations>');
